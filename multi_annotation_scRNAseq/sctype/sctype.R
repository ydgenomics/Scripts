...
# prepare gene sets
gs_list <- gene_sets_prepare(input_marker_csv, tissue)
str(gs_list)

seu <- readRDS(input_query_rds)
DefaultAssay(seu) <- "RNA" # set default assay to RNA

# 如果seu缺少data或scale.data，则需要先运行NormalizeData和ScaleData函数
if (is.null(seu[["RNA"]]@data) || nrow(seu[["RNA"]]@data) == 0) {
    message("Running NormalizeData as RNA@data is missing or empty.")
    seu <- NormalizeData(seu, normalization.method = "LogNormalize", scale.factor = 10000)
}
if (is.null(seu[["RNA"]]@scale.data) || nrow(seu[["RNA"]]@scale.data) == 0) {
    message("Running ScaleData as RNA@scale.data is missing or empty.")
...
pdf(output_umap, width = 10, height = 10)
print(p4)
dev.off()
