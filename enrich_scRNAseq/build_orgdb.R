# Date: 20250610 # Image: enrich-R--04
# Description: build_orgdb.R--This script is used to build orgdb database for peanut
# Included: Four functions: main(required), deal_go_obo(), build_orgdb(required), build_go_gmt(required), build_ko_gmt(required)
...
option_list <- list(
    ...
    make_option(c("--go_obo_csv"), type = "character", default = "/data/work/0.peanut/orgdb/test2/go_obo_result.csv", help = "Go info csv"),
...
build_go_gmt <- function(gene2go, go_obo_csv, genus, species) {
...
main <- function(
  ...
  go_ko <- build_orgdb(emapper_annotations_xlsx, kegg_info_RData, tax_id, genus, species)
  ...
  build_go_gmt(gene2go, go_obo_csv, genus, species)
  message("Starting ko_gmt build process...")
  build_ko_gmt(gene2ko, genus, species)
}

main(emapper_annotations_xlsx, kegg_info_RData, go_obo_csv, tax_id, genus, species)